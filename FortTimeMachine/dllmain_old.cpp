...
DWORD WINAPI Main(LPVOID lpParam) {
	...
	if (!pUWorldAddress) {
		...
		MessageBox(NULL, (LPCWSTR)L"Failed to find the UWorld address.", (LPCWSTR)L"Error", MB_ICONERROR | MB_OK);
		return 0;
	...
	if (!pGObjectAddress) {
		...
		MessageBox(NULL, (LPCWSTR)L"Failed to find the GObject address.", (LPCWSTR)L"Error", MB_ICONERROR | MB_OK);
		return 0;
		ExitProcess(EXIT_FAILURE);
	}
	...
	if (!pGNameAddress) {
		...
		MessageBox(NULL, (LPCWSTR)L"Failed to find the GName address.", (LPCWSTR)L"Error", MB_ICONERROR | MB_OK);
		return 0;
		ExitProcess(EXIT_FAILURE);
	}
	...
	if (!pPossessAddress) {
		...
		MessageBox(NULL, (LPCWSTR)L"Failed to find the AController::Possess function.", (LPCWSTR)L"Error", MB_ICONERROR | MB_OK);
		return 0;
		ExitProcess(EXIT_FAILURE);
	}
	...
	if (!pProcessEventAddress) {
		...
		MessageBox(NULL, (LPCWSTR)L"Failed to find the UObject::ProcessEvent function.", (LPCWSTR)L"Error", MB_ICONERROR | MB_OK);
		return 0;
		ExitProcess(EXIT_FAILURE);
	}
	...
	/*
	THIS SHIT OBVIOUSLY DOES NOT WORK AND IS PRONE TO CRASHING

	SDK::UFortCheatManager* FortCheatManager = FortPlayerController->FortCheatManager;
	if (!FortCheatManager) {
		printf("Finding CheatManager has failed, bailing out immediately!\n");
		return 0;
	}
	*/
	...
	printf("Waiting five seconds for game to process pawn possession\n");
	...
	/*
	printf("Waiting five seconds before calling ToggleCharacterBackpack();");
	
	Sleep(5000);

	THIS SHIT OBVIOUSLY DOES NOT WORK AND IS PRONE TO CRASHING
	printf("Calling ToggleCharacterBackpack");
	reinterpret_cast<SDK::UFortCheatManager*>(FortCheatManager)->ToggleCharacterBackpack();
	*/
